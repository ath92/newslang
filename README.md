...
An LLM-generated **article quiz** (multiple choice or open answer) lets readers
test their comprehension without leaving the article.
...
POST   /api/quiz          -> generate an article quiz; body { articleUrl, title?, text, mode, count }
POST   /api/quiz/grade    -> grade one open answer; body { question, answer, articleTitle? }
```
...
"off" option) on the dedicated **Settings** page (`/settings`). Opening an article
...
when the day's target is crossed. The chip in the Home and article headers shows
today's progress plus the current streak and links to the settings page, which
also shows a 7-day history strip.
...
scheduler) live in `shared/vocab.ts` and are unit-tested in `test/`. The quiz
prompt builders, schema validation and parsing live in `shared/quiz.ts` and
`worker/quiz.ts`, also unit-tested in `test/`.

### Article quiz

The reader view has a **Mettiti alla prova** button that opens a quiz docked at
the bottom of the screen, so the article stays readable while answering. The
plain-text article body (already extracted client-side by Readability) is sent
to `POST /api/quiz`, which asks **Cloudflare Workers AI** for a set of questions
and returns them one at a time:

- **Multiple choice** — the model writes three plausible Italian options and the
  client grades the pick locally.
- **Open answer** — the reader types an Italian answer, which `POST /api/quiz/grade`
  grades for both correctness and language (spelling, grammar, word choice).

The question mode (open, multiple choice or mixed) and the number of questions
are configurable in **Settings**; the default is open answer. Quizzes are
generated fresh per request and are not persisted yet — `shared/contracts.ts`
defines a `QuizResult` shape so they can be folded into progress later. The
article body is untrusted input: it is passed to the model between delimiters
with an explicit instruction to ignore any instructions inside it.

Models default to `@cf/mistralai/mistral-small-3.1-24b-instruct` (configurable
via `QUIZ_MODEL` / `QUIZ_GRADE_MODEL`). The `AI` binding is declared in
`wrangler.jsonc`; no secret is needed. Set `QUIZ_MOCK=true` in local dev to skip
the model and use a deterministic local quiz. A per-user hourly generation cap
keeps a single reader from running up an unbounded AI bill.
...
The article quiz uses the Workers AI `AI` binding declared in `wrangler.jsonc`,
so there is no secret to set. The model IDs are plain vars (`QUIZ_MODEL`,
`QUIZ_GRADE_MODEL`). Local dev reads `QUIZ_MOCK` from `.dev.vars`.
